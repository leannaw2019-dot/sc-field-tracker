...
(() => {
  ...
  ========================================================= */
  function initMaps() {
    const jobMapNode = el("jobMap");
    const setupMapNode = el("setupMap");

    if (!jobMapNode || !setupMapNode) {
      console.error("Map container missing: #jobMap or #setupMap");
      return false;
    }

    jobMap = new google.maps.Map(jobMapNode, {
    ...
    setupMap = new google.maps.Map(setupMapNode, {
      center: DEFAULT_CENTER,
      zoom: DEFAULT_ZOOM,
      mapTypeId: "hybrid",
      tilt: 0,
  ...
  function loadFieldDropdown() {
    ...
    fields.forEach((field, index) => {
      ...
      option.textContent = field.name || `Field ${index + 1}`;
      select.appendChild(option);
    });

    if (fields[Number(previousValue)]) {
  ...
  function loadEquipmentDropdown() {
    ...
    if (equipment[Number(previousValue)]) {
      select.value = previousValue;
    } else {
      select.selectedIndex = 0;
    }
  ...
  function distanceMeters(a, b) {
    if (
      ...
      return google.maps.geometry.spherical.computeDistanceBetween(
        new google.maps.LatLng(Number(a.lat), Number(a.lng)),
        new google.maps.LatLng(Number(b.lat), Number(b.lng))
      );
    }
  ...
  function getBoundaryStats(boundary) {
    ...
    for (let i = 1; i < boundary.length; i += 1) {
      perimeterMeters += distanceMeters(boundary[i - 1], boundary[i]);
    }
    ...
    if (
      ...
      const path = boundary.map((point) =>
        new google.maps.LatLng(Number(point.lat), Number(point.lng))
      );
      areaSqMeters = google.maps.geometry.spherical.computeArea(path);
    }
  ...
  function ensureFieldStats(field) {
    if (!field) {
      return {
        perimeterFeet: 0,
        areaAcres: 0
    ...
    const hasPerimeter =
      typeof field.perimeterFeet === "number" &&
      !Number.isNaN(field.perimeterFeet);

    const hasArea =
      typeof field.areaAcres === "number" &&
      !Number.isNaN(field.areaAcres);

    if (hasPerimeter && hasArea) {
      return {
        perimeterFeet: field.perimeterFeet,
        areaAcres: field.areaAcres
      };
    }

    const stats = getBoundaryStats(field.boundary || []);
    field.perimeterFeet = stats.perimeterFeet;
    field.areaAcres = stats.areaAcres;
    return stats;
  }
  ...
  function getSelectedEquipment() {
    const equipment = getEquipment();
    const equipmentSelect = el("equipmentSelect");
    if (!equipmentSelect) return null;

    const index = Number(equipmentSelect.value);
    if (!Number.isInteger(index)) return null;
    return equipment[index] || null;
  }

  function getSelectedEquipmentWidthFeet() {
    const item = getSelectedEquipment();
    return item ? Number(item.width || 0) : 0;
  }

  function getSelectedEquipmentWidthMeters() {
    return getSelectedEquipmentWidthFeet() * 0.3048;
  }

  /* =========================================================
     [13] JOB STATUS / COVERAGE UI
  ========================================================= */
  function updateJobFieldAndCoverageUI() {
    ...
    if (!field) {
      ...
      setJobCoverageStats("Covered: 0.00 acres");
      setJobCompletion("Completion: 0.0%");
      return;
    }

    const stats = ensureFieldStats(field);
    const fieldAreaSqMeters = acresToSqMeters(stats.areaAcres);
    const coveredSqMeters = Math.min(
      totalCoveredSqMeters,
      fieldAreaSqMeters || totalCoveredSqMeters
    );
    const coveredAcres = sqMetersToAcres(coveredSqMeters);

    let completionPct = 0;
    if (fieldAreaSqMeters > 0) {
      completionPct = (coveredSqMeters / fieldAreaSqMeters) * 100;
    }

    completionPct = Math.max(0, Math.min(100, completionPct));

    setJobFieldStats(
      `${field.name} • ${formatAcres(stats.areaAcres)} • ${formatFeet(stats.perimeterFeet)}`
    );
    setJobCoverageStats(`Covered: ${coveredAcres.toFixed(2)} acres`);
    setJobCompletion(`Completion: ${completionPct.toFixed(1)}%`);
  }
  ...
  function renderFieldList() {
    ...
    if (!fields.length) {
      ...
      li.textContent = "No fields saved.";
      list.appendChild(li);
      return;
    }
    ...
    fields.forEach((field, index) => {
      ...
      left.textContent = `${field.name} • ${formatAcres(ensureFieldStats(field).areaAcres)}`;

      const controls = document.createElement("div");
      controls.className = "saved-row-actions";
  ...
  function renderEquipmentList() {
    ...
    if (!equipment.length) {
      const li = document.createElement("li");
      li.textContent = "No equipment saved.";
      list.appendChild(li);
      return;
    }
  ...
  function clearJobCoverageGraphics() {
  ...
  function renderSetupPerimeterPreview() {
    ...
    if (perimeterPoints.length >= 3) {
      ...
      const stats = getBoundaryStats(autoCloseBoundary(perimeterPoints));
      setSetupFieldStats(
        `Perimeter: ${formatFeet(stats.perimeterFeet)} • Area: ${formatAcres(stats.areaAcres)}`
      );
    } else {
      setSetupFieldStats("Need at least 3 points for field size.");
    }
  ...
  function drawSelectedFieldOnJobMap() {
    ...
    if (!Array.isArray(field.boundary) || field.boundary.length < 3) {
      updateJobFieldAndCoverageUI();
      return;
    }
  ...
  function zoomSetupMapToField(index) {
    ...
    if (!field || !setupMap || !Array.isArray(field.boundary) || field.boundary.length < 3) return;
  ...
  function startPerimeterRecording() {
    ...
    clearSetupPreviewGraphics();
    setPerimeterPointCount(0);
    setSetupFieldStats("Recording new perimeter...");
  ...
  function stopPerimeterRecording() {
    ...
    if (perimeterPoints.length >= 3) {
      ...
      setSetupFieldStats("Need at least 3 points.");
    }
  }
  ...
  function saveField() {
    ...
    const stats = getBoundaryStats(closedBoundary);
    const fields = getFields();

    fields.push({
      name,
      boundary: closedBoundary,
      perimeterFeet: stats.perimeterFeet,
      areaAcres: stats.areaAcres,
      createdAt: new Date().toISOString()
    });

    setFields(fields);

    const fieldNameInput = el("fieldNameInput");
  ...
  function addEquipment() {
    ...
    const equipmentNameInput = el("equipmentNameInput");
    const equipmentWidthInput = el("equipmentWidthInput");

    if (equipmentNameInput) equipmentNameInput.value = "";
    if (equipmentWidthInput) equipmentWidthInput.value = "";
  ...
  /* =========================================================
     [20] JOB COVERAGE + DRIVEN LINE
  ========================================================= */
  function ensureJobPathLine() {
    if (!jobMap) return;
  ...
  function drawCoverageAt(latLng) {
    ...
    const widthMeters = getSelectedEquipmentWidthMeters();
    if (!widthMeters || widthMeters <= 0) return;

    const radiusMeters = Math.max(1, widthMeters / 2);

    jobPath.push(latLng);
    ensureJobPathLine();
    if (jobPathLine) jobPathLine.setPath(jobPath);

    const minSpacing = Math.max(1.5, radiusMeters * 0.5);

    if (!lastCoverageLatLng || distanceMeters(lastCoverageLatLng, latLng) >= minSpacing) {
  ...
  function clearCoverage() {
    clearJobCoverageGraphics();
    clearJobPathGraphics();
    updateJobFieldAndCoverageUI();
  }
  ...
  function bindEvents() {
    ...
    el("equipmentSelect")?.addEventListener("change", () => {
      updateJobFieldAndCoverageUI();
    });
